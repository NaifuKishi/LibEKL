...
local function _uiGridCell(name, parent) 
	...
	local cell = LibEKL.UICreateFrame ('nkCanvas', name, parent)
	cell:SetWidth(0)
	cell:SetHeight(0)
	cell:SetMouseMasking('limited')
	
	local label = LibEKL.UICreateFrame ('nkText', name .. 'label', cell)	
	...
	local borderColor = LibEKL.Art.GetThemeColor('borderColor')
	local bodyColor = LibEKL.Art.GetThemeColor('backgroundColor')
	...
	function cell:SetTexture(textureType, texturePath)
		if texture == nil then
			...
			texture = LibEKL.UICreateFrame ('nkTexture', name .. 'texture', cell)
		...
		if texturePath == nil then textureType, texturePath = 'LibEKL', 'gfx/empty.png' end
	...
	function cell:SetFont (addonId, fontName)
		LibEKL.UI.SetFont(label, addonId, fontName)
	end
