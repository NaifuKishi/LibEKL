...
local oInspectAddonCurrent = Inspect.Addon.Current
...
local function _uiGrid(name, parent) 
  ...
  local debugId
  if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL _uiGrid") end

	--if LibEKL.internal.checkEvents (name, true) == false then return nil end

	-- pre-define ui elements --
	
	local grid = LibEKL.UICreateFrame ('nkFrame', name, parent)
	local progressBar = LibEKL.UICreateFrame('nkProgressBar', name .. '.progressBar', parent)
	...
	local borderColor =  LibEKL.Art.GetThemeColor('gridColor')[1]
	local bodyColor = LibEKL.Art.GetThemeColor('gridColor')[2]
	local labelColor = LibEKL.Art.GetThemeColor('gridColor')[3]
	local headerLabelColor = LibEKL.Art.GetThemeColor('gridColor')[4]
	local labelHighlightColor = LibEKL.Art.GetThemeColor('gridColor')[5]
	local bodyHighlightColor = LibEKL.Art.GetThemeColor('gridColor')[6]
	local labelSelectedColor = LibEKL.Art.GetThemeColor('gridColor')[7]
	local bodySelectedColor = LibEKL.Art.GetThemeColor('gridColor')[8]
	...
	function grid:Layout (newCols, newRows)
		...
		local debugId
		if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:Layout") end
		...
		for idx = 1, #cols, 1 do
			local thisHeader = LibEKL.UICreateFrame('nkGridHeaderCell', name .. 'HeaderCell' .. idx, parent)
			...
			function ()
				...
				for idx = 1, rows, 1 do
					...
					for idx2 = 1, #cols, 1 do
						local thisCell = LibEKL.UICreateFrame('nkGridCell', name .. 'cell' .. idx .. '.' .. idx2, parent)
					...
					if idx == rows then
						progressBar:SetVisible(false)
						LibEKL.eventHandlers[name]["GridFinished"]()
					end
		...
		LibEKL.Coroutines.Add ({ func = gridCoRoutine, counter = rows, active = true })
		--progressText:SetVisible(true)
		
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:Layout", debugId) end
	...
	function grid:ReLayout()
		...
		local debugId
		if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:ReLayout") end
		...
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:ReLayout", debugId) end
	...
	function grid:UpdateLayout(newCols)
		...
		local debugId
		if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:UpdateLayout") end
		...
		for idx = 1, #newCols, 1 do
			...
			if thisHeader ~= nil then
				...
				LibEKL.tools.error.display ("LibEKL", 'Error in grid:UpdateLayout(): cannot increase number of cols after creation!', 1)
				return
			end
		end
		...
		LibEKL.coroutines.add ({ func = gridCoRoutine, counter = rows, callBack = function () LibEKL.eventHandlers[name]["LayoutUpdated"]() end, active = true })

		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:UpdateLayout", debugId) end
	...
	function grid:SetCellValue(row, coll, newValue)
	  ...
	  local debugId
	  if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:SetCellValue") end
		...
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:SetCellValue", debugId) end
	...
	function grid:UpdateGrid()
		...
		local debugId
		if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:UpdateGrid") end
      ...
      thisRow = idx - rowPos + 1
		...
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:UpdateGrid", debugId) end
	...
	function grid:RowHighlight (row, active)
		...
		local debugId
    if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:RowHighlight") end
		...
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:RowHighlight", debugId) end
	...
	function grid:RowSelect (row, active)
	  ...
	  local debugId
    if nkDebug then debugId = nkDebug.traceStart (oInspectAddonCurrent(), "LibEKL uiGrid:RowSelect") end
		...
		if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "LibEKL uiGrid:RowSelect", debugId) end
	...
	function grid:filter (searchValue, col, reset)
		...
		if reset ~= true then 
			if backupFilter == nil then backupFilter = LibEKL.tools.table.copy(cellValues) end
		end
    ...
    grid:UpdateGrid()
    LibEKL.eventHandlers[name]["WheelForward"](rowPos)      
	end
    ...
    if highlightedRow ~= nil then  grid:RowHighlight(highlightedRow, false) end
    grid:UpdateGrid()
    LibEKL.eventHandlers[name]["WheelBack"](rowPos)
	end
	...
	grid:EventAttach(Event.UI.Input.Mouse.Cursor.Move, function ()
		...
		LibEKL.eventHandlers[name]["MouseOver"](rowNo)
	...
	grid:EventAttach(Event.UI.Input.Mouse.Left.Click, function ()
		...
		if mouse.y < grid:GetTop() + headerHeight then
			...
			LibEKL.eventHandlers[name]["LeftClick"](rowNo)
		end
	end, name .. "LeftClick")
	...
	grid:EventAttach(Event.UI.Input.Mouse.Right.Click, function ()
		...
		LibEKL.eventHandlers[name]["RightClick"](rowNo)
	end, name .. "RightClick")
  ...
  grid:EventAttach(Event.UI.Input.Key.Down, function (_, a, key)
	...
	LibEKL.eventHandlers[name]["GridFinished"], LibEKL.Events[name]["GridFinished"] = Utility.Event.Create(addonInfo.identifier, name .. "GridFinished")
	LibEKL.eventHandlers[name]["LayoutUpdated"], LibEKL.Events[name]["LayoutUpdated"] = Utility.Event.Create(addonInfo.identifier, name .. "LayoutUpdated")
	LibEKL.eventHandlers[name]["LeftClick"], LibEKL.Events[name]["LeftClick"] = Utility.Event.Create(addonInfo.identifier, name .. "LeftClick")
	LibEKL.eventHandlers[name]["RightClick"], LibEKL.Events[name]["RightClick"] = Utility.Event.Create(addonInfo.identifier, name .. "RightClick")
	LibEKL.eventHandlers[name]["MouseOver"], LibEKL.Events[name]["MouseOver"] = Utility.Event.Create(addonInfo.identifier, name .. "MouseOver")
	LibEKL.eventHandlers[name]["MouseOut"], LibEKL.Events[name]["MouseOut"] = Utility.Event.Create(addonInfo.identifier, name .. "MouseOut")
	LibEKL.eventHandlers[name]["WheelForward"], LibEKL.Events[name]["WheelForward"] = Utility.Event.Create(addonInfo.identifier, name .. "WheelForward")
	LibEKL.eventHandlers[name]["WheelBack"], LibEKL.Events[name]["WheelBack"] = Utility.Event.Create(addonInfo.identifier, name .. "WheelBack")
	
	if nkDebug then nkDebug.traceEnd (oInspectAddonCurrent(), "_uiGrid", debugId) end
