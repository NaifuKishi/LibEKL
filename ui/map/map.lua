...
local function _uiMap(name, parent)
	...
	local ui = LibEKL.UICreateFrame("nkWindow", name .. ".window", parent)
	ui:SetWidth(425)
	ui:SetHeight(370)
	ui:SetDragable(true)
	...
	iconZoomIn:SetTextureAsync("LibEKL", "gfx/icons/zoomIn.png")
	...
	iconZoomOut:SetTextureAsync("LibEKL", "gfx/icons/zoomOut.png")
	...
	iconMinMax:SetTextureAsync("LibEKL", "gfx/icons/maximize.png")
	...
	LibEKL.UI.SetFont(coordLabel, addonInfo.id, "MontserratSemiBold")
	...
	local tooltip = LibEKL.UICreateFrame("nkTooltip", name .. ".tooltip", ui)
	...
	local function _fctRedraw ()
		...
		local debugId  
		if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:Redraw") end    
		...
		if originalScale ~= currentScale then
			...
			LibEKL.eventHandlers[name]["Zoomed"](currentScale, maximized)
		end

		if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:Redraw", debugId) end
	...
	local function _fctZoomOut()
		...
		if mapWidth >= maskWidth and mapHeight >= maskHeight then
			...
			local debugId  
			if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:ZoomOut") end
			...
			if (thisScale - scaleStep >= 0) then
				...
				_fctRedraw()
				LibEKL.eventHandlers[name]["Zoomed"](thisScale, maximized)
			end 

			if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:ZoomOut", debugId) end     
		end
	end
	...
	local function _fctZoomIn()
		...
		local debugId
		if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:ZoomIn") end
		...
		if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:ZoomIn", debugId) end
	...
	function ui:ToggleMinMax(internalFunc)
		...
		if maximized == true then
			...
			maximized = true
			iconMinMax:SetTextureAsync("LibEKL", "gfx/icons/minimize.png")
		...
		if internalFunc == true then LibEKL.eventHandlers[name]["Toggled"]() end
	end
	...
	function ui:SetMap(activeType, mapName)
		...
		mapInfo = LibEKL.Map.getMapData (mapName) 
	...
	function ui:AddElement (newElement)
		...
		local debugId 
		if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:AddElement") end
		...
		if mapData.mapElements[newElement.type] == nil then
			...
			if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:AddElement", debugId) end
			return 
		end
		...
		if mapInfo.anim ~= nil then
			...
			thisElement = LibEKL.UICreateFrame("nkMapElementCanvas", newElement.type .. "." .. LibEKLUUID(), mask)
		elseif mapInfo.gfxType == nil or stringLower(mapInfo.gfxType) == 'texture' then
			...
			thisElement = LibEKL.UICreateFrame("nkMapElementTexture", newElement.type .. "." .. LibEKLUUID(), mask)
	...
	function ui:ChangeElement (updateElement)
		...
		local debugId  
		if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:ChangeElement") end
		...
		if thisElement == nil then 
			if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:ChangeElement", debugId) end
			return false 
	...
	function ui:RemoveElement(removeElement)
		...
		local debugId  
		if nkDebug then debugId = nkDebug.traceStart (inspectAddonCurrent(), "LibEKL _uiMap:RemoveElement") end
		...
		if nkDebug then nkDebug.traceEnd (inspectAddonCurrent(), "LibEKL _uiMap:RemoveElement", debugId) end
...
--[[
	Command.Event.Attach(LibEKL.Events[name .. '.window'].Moved, function (_, newX, newY)
		...
		LibEKL.eventHandlers[name]["Moved"](newX, newY, maximized)
	...
	Command.Event.Attach(LibEKL.Events[name .. '.window'].Resized, function (_, newWidth, newHeight)
		...
		LibEKL.eventHandlers[name]["Resized"](newWidth, newHeight, maximized)
	...
	ui:GetContent():EventAttach(Event.UI.Input.Mouse.Cursor.Out, function ()
		LibEKL.eventHandlers[name]["MouseMoved"]("")
	...
	ui:GetContent():EventAttach(Event.UI.Input.Mouse.Cursor.Move.Bubble, function (self, _, posX, posY)
		...
		if drag ~= true then
			...
			--print ("hossa")
			LibEKL.eventHandlers[name]["MouseMoved"](coordLabel:GetText())
			return 
		end
	...
	LibEKL.eventHandlers[name]["MouseMoved"], LibEKL.Events[name]["MouseMoved"] = Utility.Event.Create(addonInfo.identifier, name .. "MouseMoved")
	LibEKL.eventHandlers[name]["Resized"], LibEKL.Events[name]["Resized"] = Utility.Event.Create(addonInfo.identifier, name .. "Resized")
	LibEKL.eventHandlers[name]["Zoomed"], LibEKL.Events[name]["Zoomed"] = Utility.Event.Create(addonInfo.identifier, name .. "Zoomed")
	LibEKL.eventHandlers[name]["Toggled"], LibEKL.Events[name]["Toggled"] = Utility.Event.Create(addonInfo.identifier, name .. "Toggled")
