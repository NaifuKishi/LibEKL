...
local function _uiMapElementCanvas(name, parent)
	...
	function mapElement:RegisterAnimation (thisAnimationSpeed)
		...
		if parentMap:GetAnimated() == true then
			if thisMapData.anim == "rotation" then
				...
				LibEKL.fx.register (effectName, mapElement, {id = "rotateCanvas", speed = (thisAnimationSpeed or 0), scale = scale, path = path, fill = fill  })
			else
				effectName = nil
			end
		end
	...
	function mapElement:SetVisible(flag)
		...
		if effectName then
			if flag then
				LibEKL.fx.updateTime(effectName)
			else
				LibEKL.fx.pauseEffect(effectName)
			end
		end
	end
