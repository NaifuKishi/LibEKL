...
if not LibEKL.Map then LibEKL.Map = {} end
...
local LibEKLStringsSplit		= LibEKL.strings.split
...
local function _fctCheckForColossus (values)
  ...
  if _mapColossus == nil then
    ...
    if _mapColossus == nil then
      LibEKL.Tools.Error.Display ("LibEKL", "Could not unzip colossus data", 1)
      return values.type
    end 
  end
 
--	LibEKLDebug = { colossus = _mapColossus }
 
  for k, v in pairs (_mapColossus) do
    
    local name = v[LibEKLGetLanguageShort()]
...
local function _fctCheckForNPC (values)
	...
	local zone = LibEKL.Map.getZoneByUnit (LibEKL.Unit.GetPlayerDetails().id)
	...
	if _mapNPC [zone] == nil then
		...
		if _mapNPC[zone] == nil then
			LibEKL.Tools.Error.Display ("LibEKL", "Could not unzip npc data", 1)
			return false
		end 
	end

	for k, v in pairs (_mapNPC[zone]) do
		if values == v[LibEKLGetLanguageShort()] then
			return true
		end	
	end
...
local function _fctIdentify(values)
	...
	if values.title ~= nil then 
		...
		values.titleList = LibEKLStringsSplit(values.title, "\n")
	...
	if values.description ~= nil then
		...
		values.descList = LibEKLStringsSplit(values.description, "\n")
	...
	if titleIndex == 0 and descIndex == 0 then
		if nkDebug then
			LibEKL.Tools.Error.Display ("LibEKL", "Could not identify map entry", 2)
...
local function _fctMapEventRemove (_, info)
  ...
  if hasRemoves == true then LibEKL.eventHandlers["LibEKL.Map"]["remove"](temp) end
...
local function _fctMapEventChange (_, info)
  ...
  if newlyAdded > 0 then
    internalFunc.mapEvent.add(_, addList)
  ...
  if descTitleChange == true then
    ...
    LibEKL.eventHandlers["LibEKL.Map"]["add"](addList)
  end
  
  if hasChange == true then
    LibEKL.eventHandlers["LibEKL.Map"]["change"](changeList)
  end
...
local function _fctMapEventCoord  (_, x, y, z)
  ...
  if hasAdd == true then internalFunc.mapEvent.add (_, addList) end
  if hasChanges == true then LibEKL.eventHandlers["LibEKL.Map"]["coord"](changeList) end
...
local function _fctMapEventUnitCoordChange (_, x, y, z)
  ...
  if hasAdd then LibEKL.eventHandlers["LibEKL.Map"]["unitAdd"](addUnit) end
  if hasChange then LibEKL.eventHandlers["LibEKL.Map"]["unitChange"](changeUnit) end
...
local function _fctMapEventUnitUnavailable (_, info)
  ...
  for k, v in pairs (info) do
    if _mapUnits[k] ~= nil then
      ...
      _mapUnits[k] = nil
      if _wayPoints[k] ~= nil then internalFunc.MapEventWaypoint (_, {[k] = true}) end
      hasRemoves = true
    end
  end 
  
  if hasRemoves == true then LibEKL.eventHandlers["LibEKL.Map"]["unitRemove"](removeUnit) end
...
local function _fctMapEventUnitAvailable (_, info)
  ...
  if hasAdds == true then LibEKL.eventHandlers["LibEKL.Map"]["unitAdd"](addUnit) end
...
local function _fctMapEventCombatDeath (_, info)
  ...
  if info.target == data.playerDetails.id then
    ...
    LibEKL.eventHandlers["LibEKL.Map"]["add"](addInfo)
  end
...
local function _fctMapEventUnitHealth (_, info)
  ...
  _playerDeath = false
  LibEKL.eventHandlers["LibEKL.Map"]["remove"]({["pb" .. data.playerDetails.id] = true})
  
end 

---------- library public function block ---------

function LibEKL.Map.replaceData (key, data)        mapData.mapData[key] = data     end
function LibEKL.Map.replaceMapElement (key, data)  mapData.mapElements[key] = data end
function LibEKL.Map.getAll()                       return _mapPoints, _mapUnits    end
function LibEKL.Map.addMapElement (key, data)      mapData.mapElements[key] = data end
function LibEKL.Map.getMapData (id)					return mapData.mapData[id] end

function LibEKL.Map.GetMapElementbyType (typeString)
...
function LibEKL.Map.clearAll()
  _mapPoints = {}
  _mapUnits = {}
end

function LibEKL.Map.refresh()
  
  LibEKL.Map.clearAll()
  internalFunc.mapEvent.add (_, InspectMapList())

end

--function LibEKL.Map.getMapElements ()              return data.mapElements                               end

function LibEKL.Map.Init(flag)

  if flag == true and _mapEvents == false then
    Command.Event.Attach(Event.Map.Add, internalFunc.mapEvent.add, "LibEKL.Map.Add")
    Command.Event.Attach(Event.Map.Change, _fctMapEventChange, "LibEKL.Map.Change")
    Command.Event.Attach(Event.Map.Remove, _fctMapEventRemove, "LibEKL.Map.Remove")
    Command.Event.Attach(Event.Map.Detail.Coord, _fctMapEventCoord, "LibEKL.Map.Detail.Coord")
    Command.Event.Attach(Event.Map.Waypoint.Update, internalFunc.MapEventWaypoint, "LibEKL.Map.Waypoint.Update")
    Command.Event.Attach(Event.Unit.Detail.Coord, _fctMapEventUnitCoordChange, "LibEKL.Map.Unit.Detail.Coord")
    Command.Event.Attach(Event.Unit.Detail.Health, _fctMapEventUnitHealth, "LibEKL.Map.Unit.Detail.Health")    
    Command.Event.Attach(Event.Unit.Availability.None, _fctMapEventUnitUnavailable, "LibEKL.Map.Unit.Unavailable")
    Command.Event.Attach(Event.Unit.Availability.Full, _fctMapEventUnitAvailable, "LibEKL.Map.Unit.Full")
    Command.Event.Attach(Event.Combat.Death, _fctMapEventCombatDeath, "LibEKL.Map.Combat.Death")    
  elseif flag == false and _mapEvents == true then
    Command.Event.Detach(Event.Map.Add, nil, "LibEKL.Map.Add")
    Command.Event.Detach(Event.Map.Change, nil, "LibEKL.Map.Change")
    Command.Event.Detach(Event.Map.Remove, nil, "LibEKL.Map.Remove")
    Command.Event.Detach(Event.Map.Detail.Coord, nil, "LibEKL.Map.Detail.Coord")
    Command.Event.Detach(Event.Unit.Detail.Health, nil, "LibEKL.Map.Unit.Detail.Health")
    Command.Event.Detach(Event.Map.Waypoint.Update, nil, "LibEKL.Map.Waypoint.Update")
    Command.Event.Detach(Event.Unit.Detail.Coord, nil, "LibEKL.Map.Unit.Detail.Coord")
    Command.Event.Detach(Event.Unit.Availability.None, nil, "LibEKL.Map.Unit.Unavailable") 
    Command.Event.Detach(Event.Unit.Availability.Full, nil, "LibEKL.Map.Unit.Full")
    Command.Event.Attach(Event.Combat.Death, nil, "LibEKL.Map.Combat.Death")
  end
  
  Command.Map.Monitor(flag)
  _mapEvents = flag
  
  if flag == true then internalFunc.mapEvent.add (_, InspectMapList()) end

end

---------- addon internalFunc function block ---------

function internalFunc.processMap()

	local debugId  
	if nkDebug then debugId = nkDebug.traceStart (InspectAddonCurrent(), "LibEKL internalFunc.processMap") end

	if _mapEvents == false then return end

	if data.playerDetails ~= nil and _wayPoints[data.playerDetails.id] ~= nil then
		internalFunc.MapEventWaypoint (_, {[data.playerDetails.id] = true})
	end
	...
	if hasAdds then internalFunc.mapEvent.add (_, addList) end
	...
	if nkDebug then nkDebug.traceEnd (InspectAddonCurrent(), "LibEKL internalFunc.processMap", debugId) end

end

function internalFunc.MapEventWaypoint (_, info)
	...
	if hasRemove == true then LibEKL.eventHandlers["LibEKL.waypoint"]["remove"](removes) end
	if hasAdd == true then LibEKL.eventHandlers["LibEKL.waypoint"]["add"](add) end
	if hasChange == true then LibEKL.eventHandlers["LibEKL.waypoint"]["change"](change) end
  
end

function internalFunc.mapEvent.add (_, info)
  ...
  if hasAdds then LibEKL.eventHandlers["LibEKL.Map"]["add"](addList) end
  if hasChanges then LibEKL.eventHandlers["LibEKL.Map"]["change"](changeList) end
