...
local function _fctZoneEvent (_, info)
  ...
  LibEKL.eventHandlers["LibEKL.Map"]["zone"](info)
...
local function _fctZoneCheckUnit (_, info, raiseEvent)
  ...
  if raiseEvent ~= false then LibEKL.eventHandlers["LibEKL.Map"]["zone"](eventData) end  

end

---------- addon internalFunc function block ---------

function internalFunc.checkShard()

	local debugId  
    if nkDebug then debugId = nkDebug.traceStart (InspectAddonCurrent(), "LibEKL internalFunc.checkShard") end
        ...
        for name, _ in pairs(consoleData.channel) do
          if name == zone.name then
            if _shardName ~= InspectShard().name then
              ...
              LibEKL.eventHandlers["LibEKL.Map"]["shard"](_shardName)
            end
            return
  ...
  if nkDebug then nkDebug.traceEnd (InspectAddonCurrent(), "LibEKL internalFunc.checkShard", debugId) end	
  
end

---------- library public function block ---------

function LibEKL.Map.calcZoneList()

  LibEKLSetup.zoneList = {}
  
  for world, zones in pairs(_zoneMapping) do
    for idx = 1, #zones, 1 do
      local detail = InspectZoneDetail(zones[idx])
      LibEKLSetup.zoneList[zones[idx]] = { world = world, zone = detail.name, type = detail.type }
    end
  end

end

function LibEKL.Map.ZoneInit(flag)
  ...
  if flag == true and _zoneEvents == false then
    ...
    Command.Event.Attach(Event.Unit.Detail.Zone, _fctZoneEvent, "LibEKL.Zone.Unit.Detail.Zone")
    Command.Event.Attach(Event.Unit.Availability.Full, _fctZoneCheckUnit, "LibEKL.Zone.Unit.Availability.Full")
    
    _fctZoneCheckUnit (_, InspectUnitList(), false)
  elseif flag == false and _zoneEvents == true then
    Command.Event.Detach(Event.Unit.Detail.Zone, nil, "LibEKL.Zone.Unit.Detail.Zone")
    Command.Event.Detach(Event.Unit.Availability.Full, nil, "LibEKL.Zone.Unit.Availability.Full")
  end
...
function LibEKL.Map.getZoneByUnit (unitID)
	
	return _zoneData[unitID]
	
end

function LibEKL.Map.getZoneWorld (zoneID)
...
function LibEKL.Map.GetZonePOI (zoneID)
  ...
  if poiList ~= nil then
    ...
    for k, v in pairs(poiList) do
      poiList[k].id = k
      poiList[k].title = v[LibEKL.Tools.Lang.GetLanguageShort ()]
    end
  end
...
function LibEKL.Map.GetZoneByName (name)
...
function LibEKL.Map.GetZoneDetails (zoneID)
